...
BinarySearchTree.uniqueBST = function(n, memo = {}) {
  /**
    return Total number of unique binary search trees with values 1 to n.
    n = 0 => 1
    n = 1 => 1
    n = 2 => 2
    n = 3 => 5
  ...
  **/
  ...
  if (memo[n]) {
    ...
    for (let i = 0; i < n; i++) {
      let leftSubtrees = i;
      let rightSubtrees = n - i - 1;

      possibilities += BinarySearchTree.uniqueBST(leftSubtrees, memo) * BinarySearchTree.uniqueBST(rightSubtrees, memo);
    }
    memo[n] = possibilities;
...
console.log(BinarySearchTree.uniqueBST(4));
